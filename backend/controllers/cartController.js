...
const updateCartItem = async (req, res) => {
  ...
  const isProductAlreadyInCart = userCart.cart.findIndex(
    (cartProduct) => updateProduct.product.toString() === cartProduct.product.toString()
  );
  ...
  const temp =  {
    ...(userCart.cart[isProductAlreadyInCart]),
    ...updateProduct
  };
  await (userCart.cart[isProductAlreadyInCart]=  temp);
  await userCart.save();
  return res.status(StatusCodes.NO_CONTENT).json({ msg:"Updation Succesfull" });
};
