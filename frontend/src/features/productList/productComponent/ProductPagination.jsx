...
export default function ProductPagination({ totalItems }) {
  const page = useSelector((state) => state.product.page);
  const dispatch = useDispatch();
  const totalPages = Math.ceil(totalItems / productListLimit);
  const displayPages = paginationDisplay(page, totalPages);
  ...
  return (
    ...
    <div className="flex items-center justify-between  border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
      <div className="flex flex-1 justify-between sm:hidden">
        ...
        <button
          onClick={() => dispatch(incrementPagePage())}
        ...
        <div key={"ajdsdfijasf"}>
          ...
          >
              ...
              {displayPages.map((displayPage, index) => {
                ...
                } else if (Number(displayPage) === page) {
                  return (
                    <button
